...
bot.action("PRODUCTS", async (ctx) => {
  try {
    ...
    if (products.length === 0) {
      ctx.reply("🚫 Şu anda sistemde hiç ürün bulunmamaktadır.");
      return;
    }
    ...
    const productList = products
      .map(
        (product, index) =>
          `🛒 ${index + 1}. ${product.name} | 💰 ${product.price} TL`
      )
      .join("\n");

    await ctx.reply(
      `📋 Ürün Listesi:\n${productList}\n\nDaha fazla bilgi almak için bir ürün seçebilirsiniz.`
    );
  } catch (error) {
    console.error("Hata:", error);
    ctx.reply("❌ Ürünler listelenirken bir hata oluştu.");
  }
});
